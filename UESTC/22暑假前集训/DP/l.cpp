...
signed main() {
  ...
  Segment_Tree T(-1e9, 1e9);
  
  for (int i = 1; i <= n; i++) {
    dp[i] = i - 1;
    dp[i] = min(dp[i], T.query(0, -1e9, a[i]).minn + i); 
    int old = T.query(0, a[i], a[i]).minn;
    if (old == INF) {
      T.modify(0, a[i], dp[i] - i - 1);
    } else if (old > dp[i] - i - 1) {
      T.modify(0, a[i], -old + dp[i] - i - 1);
    }
  }
  int ans = INF;
  for (int i = 1; i <= n; i++) {
