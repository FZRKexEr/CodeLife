...
//  file();
  ...
  while (cin >> n) {
    ...
    Hash S(s), T(t);
    int flag = true;
    if (S.get_hash(1, n) == T.get_hash(1, n)) flag = false;
    for (int i = 1; i < n; i++) {
      auto h1 = S.get_hash(n - i + 1, n);     
      auto h2 = T.get_hash(1, i);
      auto h3 = T.get_hash(i + 1, n);
      auto h4 = S.get_hash(1, n - i);
      if (h1 == h2 && h3 == h4) flag = false;
    }
