...
//      for (int j = 1; j <= n; j++) {
//        cout << T1.query(1, j, j).sum << " ";
//      }
//      cout << endl;
//      for (int j = 1; j <= n; j++) {
//        cout << T2.query(1, j, j).sum << " ";
//      }
//      cout << endl;
      ...
      if (x == 1) {
        int val, o; cin >> o >> val;
        T1.modify(1, o, o, -a[o] + val); 
        a[o] = val;

      } else if (x == 2) {
        int o, val; cin >> o >> val;
        int old = T2.query(1, o, o).sum;
        if (o - 1 >= 1) T1.modify(1, 1, o - 1, -old + val);
        T2.modify(1, o, o, -old + val);

      } else {
        int o; cin >> o;
        int ans = T2.query(1, 1, o).sum;
        int val =  T1.query(1, 1, o).maxi;
        if (o + 1 <= n) val -= T2.query(1, o + 1, n).sum;
        ans = max(ans, val);
        cout << ans << endl;
      }
    }
