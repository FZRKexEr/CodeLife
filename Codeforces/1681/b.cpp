...
struct Treap {
  ...
  void push_down(int pos) {
    if (tree[pos].lazy) {
      ...
      if (tree[pos].l != -1) tree[tree[pos].l].lazy ^= 1;
      if (tree[pos].r != -1) tree[tree[pos].r].lazy ^= 1;
      tree[pos].lazy = 0;
    }
  }
...
//  for (int i = 1; i <= m; i++) {
//    int l, r; cin >> l >> r;
//    array<int, 2> res1 = T.split_sz(T.root[1], l - 1);
//    array<int, 2> res2 = T.split_sz(res1[1], r - l + 1);
//    T.tree[res2[0]].lazy ^= 1;
//    T.root[1] = T.merge(res1[0], T.merge(res2[0], res2[1]));
//  }
//  vector<int> ans = T.output(T.root[1]);
