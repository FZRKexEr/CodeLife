...
int solve(vector<array<int, 2>> a) {
  ...
  T1.modify(1, n, n, 0);
  T2.modify(1, n, n, 0 + a[n][0]);

  const int INF = 0x3f3f3f3f3f3f3f3f; 
  for (int i = n - 1; i >= 1; i--) {
    int p = upper_bound(a.begin() + 1, a.end(), (array<int, 2>) {a[i][0] + a[i][1], INF}) - a.begin() - 1; 
    int res = INF;
    if (p != i) res = min(res, T1.query(1, i + 1, p).mini); 
    if (p != n) res = min(res, T2.query(1, p + 1, n).mini - a[i][0] - a[i][1]); 

    T1.modify(1, i, i, res); 
    T2.modify(1, i, i, res + a[i][0]);
  }
  
  return T1.query(1, 1, 1).sum; 
}
...
signed main() {
  ...
  vector<array<int, 2>> a1(1, {0, 0}), a2(1, {0, 0}); 
